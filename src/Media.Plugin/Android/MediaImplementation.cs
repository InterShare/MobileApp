...
namespace Plugin.Media
{
    ...
    public class MediaImplementation : IMedia
    {
        ...
        public async Task<MediaFile> PickPhotoAsync(PickMediaOptions options = null, CancellationToken token = default(CancellationToken))
        {
            ...
            MediaFile media = await TakeMediaAsync("image/*", Intent.ActionPick, null, token);
        ...
        public async Task<List<MediaFile>> PickPhotosAsync(PickMediaOptions options = null, MultiPickerOptions pickerOptions = null, CancellationToken token = default(CancellationToken))
		{
			...
			var medias = await TakeMediasAsync("image/*", Intent.ActionPick, new StorePickerMediaOptions { MultiPicker = pickerOptions?.MaximumImagesCount > 1 }, token);
        ...
        public async Task<List<MediaFile>> PickPhotoOrVideoAsync(PickMediaOptions options = null, MultiPickerOptions pickerOptions = null, CancellationToken token = default(CancellationToken))
        {
	        if (!await RequestStoragePermission())
	        {
		        return null;
	        ...
	        var medias = await TakeMediasAsync("video/* image/*", Intent.ActionPick, new StorePickerMediaOptions { MultiPicker = false }, token);
		...
		public async Task<MediaFile> TakePhotoAsync(StoreCameraMediaOptions options, CancellationToken token = default(CancellationToken))
        {
            ...
            MediaFile media = await TakeMediaAsync("image/*", MediaStore.ActionImageCapture, options, token);
            ...
            {
                string fileName = System.IO.Path.GetFileName(media.Path);
                Uri uri = MediaPickerActivity.GetOutputMediaFile(context, options.Directory ?? "temp", fileName, true, true);
                ...
                try
                {
                    ...
                    Uri albumUri = cr.Insert(MediaStore.Images.Media.ExternalContentUri, values);
        ...
        public async Task<List<MediaFile>> PickVideosAsync(CancellationToken token = default(CancellationToken))
        {
            ...
            var medias = await TakeMediasAsync("video/*", Intent.ActionPick, new StorePickerMediaOptions { MultiPicker = false }, token);
		...
		async Task<bool> RequestCameraPermissions()
		{
			...
			PermissionStatus hasStoragePermission = await Permissions.CheckStatusAsync<StoragePermission>();
			...
			string camera = nameof(Permissions.Camera);
			string storage = nameof(StoragePermission);
        ...
        async Task<bool> RequestStoragePermission()
        {
			...
			PermissionStatus status = await Permissions.CheckStatusAsync<StoragePermission>();
            if (status != PermissionStatus.Granted)
            {
                Console.WriteLine("Does not have storage permission granted, requesting.");
                PermissionStatus result = await Permissions.RequestAsync<StoragePermission>();
		...
		bool HasPermissionInManifest(string permission)
		{
			try
			{
				...
				Context permissionContext = Xamarin.Essentials.Platform.CurrentActivity ?? Application.Context;
		...
		public Task<bool> ResizeAsync(string filePath, StoreCameraMediaOptions mediaOptions, ExifInterface exif)
        {
            ...
            try
            {
				PhotoSize photoSize = mediaOptions.PhotoSize;
				int customPhotoSize = mediaOptions.CustomPhotoSize;
				int quality = mediaOptions.CompressionQuality;
                return Task.Run(() =>
                {
                    try
                    {
                        ...
                        //this now will return the requested width/height from file, so no longer need to scale
                        using (Bitmap originalImage = BitmapFactory.DecodeFile(filePath, options))
                        {
