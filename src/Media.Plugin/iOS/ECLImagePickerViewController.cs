...
namespace Plugin.Media
{
    ...
    public class ELCImagePickerViewController : UINavigationController
    {
        ...
        MediaFile GetPictureMediaFile(ALAsset asset, long index = 0)
        {
            ...
            CGImage cgImage = rep.GetImage();

            UIImage image = null;
            if (cgImage == null)
            {
                PHFetchResult fetch = PHAsset.FetchAssets(new[] { asset.AssetUrl }, null);
        ...
        {
            bool shouldSelect = MaximumImagesCount <= 0 || previousCount < MaximumImagesCount;
            if (!shouldSelect)
            {
                NSString first = NSBundle.MainBundle.GetLocalizedString("Only", "Only");
                NSString second = NSBundle.MainBundle.GetLocalizedString("photos please", "photos please!");

                var title = $"{first} {MaximumImagesCount} {second}";

                NSString third = NSBundle.MainBundle.GetLocalizedString("Please do not pick more than", "Please do not pick more than");
                NSString fourth = NSBundle.MainBundle.GetLocalizedString("photos at a time", "photos at a time.");
        ...
        public class ELCAlbumPickerController : UITableViewController
        {
            ...
            {
                base.ViewDidLoad();
                string loading = string.IsNullOrWhiteSpace(LoadingTitle) ? NSBundle.MainBundle.GetLocalizedString("Loading", "Loading...") : LoadingTitle;
            ...
            {
                ELCImagePickerViewController parent = Parent;
                if (parent != null)
                {
            ...
            void ReloadTableView()
            {
                ...
                string selectAlbum = string.IsNullOrWhiteSpace(SelectAlbumTitle) ? NSBundle.MainBundle.GetLocalizedString("Select an Album", "Select an Album") : SelectAlbumTitle;
            ...
            public override UITableViewCell GetCell(UITableView tableView, NSIndexPath indexPath)
            {
                ...
                // Get count
                ALAssetsGroup g = assetGroups[indexPath.Row];
            ...
            {
                ALAssetsGroup assetGroup = assetGroups[indexPath.Row];
                ...
                string backButtonTitle = string.IsNullOrWhiteSpace(BackButtonTitle) ? NSBundle.MainBundle.GetLocalizedString("Back", "Back") : BackButtonTitle;
        ...
        class ELCAssetTablePicker : UICollectionViewController
        {
            ...
            {
                int selectionCount = collectionView.GetIndexPathsForSelectedItems().Length;
                var shouldSelect = true;
                ALAsset asset = AssetForIndexPath(indexPath);
            ...
            void AssetSelected(NSIndexPath targetIndexPath, bool selected)
            {
                if (ImmediateReturn)
                {
                    ALAsset asset = AssetForIndexPath(targetIndexPath);
                    MediaFile mediaFile = Parent?.GetPictureMediaFile(asset);
                    asset?.Dispose();
                    asset = null;
                    if (mediaFile != null)
                    {
            ...
            void PreparePhotos()
            {
                ...
                dispatcher.BeginInvokeOnMainThread(() =>
                {
                    ...
                    // scroll to bottom
                    nint section = NumberOfSections(CollectionView) - 1;
                    nint row = CollectionView.NumberOfItemsInSection(section) - 1;
                    if (section >= 0 && row >= 0)
                    {
                        NSIndexPath ip = NSIndexPath.FromRowSection(row, section);
            ...
            async void DoneClicked(object sender = null, EventArgs e = null)
            {
                ...
                int selectedItemsCount = selectedItemsIndex.Length;
                ...
                for (var i = 0; i < selectedItemsCount; i++)
                {
                    ...
                    Task t = Task.Run(() =>
                    {
                        ALAsset alAsset = AssetForIndexPath(selectedItemsIndex[j]);
                        MediaFile mediaFile = parent?.GetPictureMediaFile(alAsset, j);
                        if (mediaFile != null)
                        {
